...
float sensirion::readHumidity()
{
  ...
  if ( millis() - _lastHumMillis < SHT_CACHE_MILLIS){
	  ...
	  if ( sendCommandSHT(_gHumidCmd, _dataPin, _clockPin) == SHT_SUCCESS ){
		  ...
		  if (mycrc != rxcrc){
        Serial.println(F("SHT: crc error"));
        Serial.print(F(" Got:      0x")); Serial.println(rxcrc, HEX);
        Serial.print(F(" Expected: 0x")); Serial.println(mycrc, HEX);
        return NAN;
		  }
...
uint8_t sensirion::readStatus()
{
  ...
  */
  ...
  if (rx_crc != rev8bits(local_crc)){
	Serial.println(F("SHT: crc error"));
	Serial.print(F(" Got:      0x")); Serial.println(rx_crc, HEX);
	Serial.print(F(" Expected: 0x")); Serial.println(local_crc, HEX);
  }
...
{
  (void)_bitorder;  //mod to avoid compilation warnings
  uint8_t ret = 0;
  uint8_t i;
  uint8_t _numBits = 8;
  ...
  {
     digitalWrite(_clockPin, HIGH);
     shtDelay();  // I don't know why I need this, but without it I don't get my 8 lsb of temp
...
void sensirion::waitForResultSHT(int _dataPin)
{
  ...
  if (ack == HIGH) {
    Serial.println(F("Ack Error 2")); // Can't do serial stuff here, need another way of reporting errors
  }
}
